...
| Evaluator dup-warning fix (agent-level READ THIS CAREFULLY removed; EVALUATE-mode copy preserved) (v2.3) | Rajasekaran 2026 | *"Stripping away pieces that are no longer load-bearing."* The leniency warning fired for all three modes; only EVALUATE has numeric leniency to bias. REVIEW-PROPOSAL has rubber-stamping (covered separately at evaluator.md:179), REVALIDATE is binary (covered at evaluator.md:276). |
...
| Per-FR story files — BMAD V6 hyper-detailed pattern (FR-4) **(removed v2.2)** | BMAD V6 Scrum Master agent | BMAD V6 ships per-story files containing "everything the Dev agent needs — full context, implementation details, and architectural guidance embedded directly in story files." Adopted as `features/NNN/stories/FR-NNN.md`, with strict no-paraphrase invariant against the aggregate contract. **Removed v2.2** under the same "every component encodes an assumption that may go stale" rule — Opus 4.7[1m] absorbs the full contract trivially, removing the need for per-FR scoping; see decision row "Stale-assumption pruning of per-FR story files (v2.2)" above. |
