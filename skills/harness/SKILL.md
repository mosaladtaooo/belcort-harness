---
name: harness
description: BELCORT Planner → Generator → Evaluator pipeline. Invoke when the user runs /harness:sprint, /harness:quick, /harness:resume, or any /harness:* slash command, or when the user describes a substantial build task (3+ components, >15 minutes of work) and has not yet activated the harness. Orchestrates planning, contract negotiation, TDD implementation, adversarial evaluation, and retrospective drift analysis via fresh subagents communicating through files in .harness/.
---
