...
- **Planner Pass 2 brainstorming pass** over user-journey ACs. Emits `## UI-Surface Audit` to prd.md. Self-validation grew 16 → 18 points (V17 + V18).
- **Constitution catch-block ban** — new MUST principle injected into Planner Pass 1 constitution-generation directive (the canonical baseline grew from 17 → 18 default principles). Existing v2.x projects' constitutions unaffected per post-init immutability — opt-in via `/harness:constitution-amend`.
...
- **`commands/sprint.md` Step 3.5 Simulate.** New step between Step 3 (BUILD) and Step 4 (EVALUATE). Phase transitions: building → simulating → evaluating.
- **Manifest `state.phase` enum gains `simulating`.**
- **`commands/resume.md` + `commands/rewind.md`** handle the new phase. SKILL.md § Recovery gains the `simulating` phase-recovery branch (resume.md is a thin pointer that delegates to SKILL.md).
- **`SKILL.md § Prompt-Injection Defense (shared)`** — preamble factored from per-agent HANDLING FETCHED CONTENT sections (Scan A). Per-agent sections become delta-only.
- **`docs/v3-scan-E-findings.md`** (new file). Command-set redundancy review yielded null result; documented for traceability.

### Changed

- **`agents/evaluator.md` MODE: EVALUATE Step 2** rewrites from "primary functional Playwright pass" to "Read simulation-report.md (Step 2a) + 15-min independent spot-check (Step 2b)". SIMULATE owns prod-mode runtime; Evaluator stops re-spinning the same stack.
- **Per-agent `## HANDLING FETCHED CONTENT` sections** become delta-only against new SKILL.md baseline (Scan A factoring). Each per-agent section now ≤18 lines.
- **`hooks/pre-tool-use.sh` test-file-deletion hard-block** downgraded to advisory log entry (Scan C). Evaluator's git-archaeology Steps A, B, C, E continue scanning for the same patterns; hook records test-deletion attempts to changelog without blocking. Other 4 hard-blocks (force-push, force-push -f, .harness/ deletion, sudo) preserved unchanged. Reversible if a test-deletion incident surfaces.

### Removed

- **Manifest dead fields (Scan D, ~16 lines net):** `harness.{model, created}`, `project.{description, created}`, `config.testing.{unit, e2e}`, `config.context_strategy`, `config.calibration_metrics.last_sprint_ratio`, `tuning_debt.patterns_pending`, entire `git.*` block (`main_branch`, `merge_strategy`, `worktree_prefix`). Confirmed via grep — zero reads outside the manifest itself.
- **Stale `templates/features/contract.md.txt:7` sync comment** referring to a no-longer-existing inline copy in generator.md (Scan B). The v2.1.3 refactor moved the contract template to `@templates/...` references; the comment was a leftover artefact.

### Migration

- **Existing v2.x contracts** (without new tables) continue to work for in-flight sprints. New sprints produce the tables. Graceful migration.
...
- **1M-context dependency elevated.** SIMULATE adds one Generator dispatch per sprint; cumulative regression cost compounds with shipped features. Without `claude --model claude-opus-4-7[1m]`, projects with ≥5 shipped features risk truncation. ROADMAP item "Mechanical 1M-context detection" elevates priority.
- **v3.1 follow-ups surfaced during implementation:** Standardize test-fixture lifecycle (currently informal per-project), worker readiness signal, real-time worker log tailing during SIMULATE, test-account convention. Tracked in ROADMAP.md.
