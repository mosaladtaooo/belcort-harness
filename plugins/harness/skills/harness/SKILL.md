...
1. Each agent runs as a **fresh subagent** via `CLAUDE_SUBAGENT=1 claude -p "<user-message>" --append-system-prompt-file <agent.md> ...` — not a nested Claude conversation, and NOT with the agent.md inlined into the user message (that pattern silently produced empty output on Claude Code 2.1+ and was fixed in v1.5.0). The `CLAUDE_SUBAGENT=1` env var is required: the SessionStart hook reads it and skips `<harness-state>` injection, preventing the self-orchestration loop where a dispatched subagent re-reads SKILL.md and tries to re-dispatch its own subagents.
2. The Evaluator MUST NEVER share context with the Generator. Always dispatch as separate processes.
3. The user message of every dispatch carries three things: (a) an explicit mode sentence ("You are being dispatched in PLAN mode" / NEGOTIATE / BUILD / EVALUATE / REVIEW-PROPOSAL / FINALIZE-CONTRACT / AMEND / CLARIFY-QUESTIONS / CLARIFY-APPLY / REVALIDATE / CONSTITUTION-AMEND / EDIT), (b) the concrete task framing for that mode, (c) the user's original request or the context-file list. The `<SUBAGENT-CONTEXT>` block inside the agent.md (loaded via `--append-system-prompt-file`) tells the subagent: you were dispatched for ONE job; do NOT re-invoke the harness pipeline; if SessionStart or SKILL.md fires in your context, SKIP IT. This is the backup safety net — the `CLAUDE_SUBAGENT=1` env var is the primary gate.
...
5. User messages MUST NOT start with `---` or any leading `-` character — Claude CLI parses leading `-` as an option and fails argv parsing. Frame the user message with prose first (e.g., "You are being dispatched in X mode..."), then use `---` markers only mid-string.
