...
6. **Round budget check (HARD CAP at 5) — manifest cross-check (Agent 3 + 4 finding A5; round-2 hardening; round-3 FIX #5 normalization)**: also Read `.harness/manifest.yaml` and extract `state.design_reroll_round` (default 0 if absent — legacy manifests). The orchestrator increments this field via `/harness:design reroll` Step 5 and resets it via `/harness:design explore` Step 0; it tracks the same round count from a place the user can't trivially edit. Cross-check against the file-based count from Step 1.5:
   - **Philosophy — the budget is a soft focus heuristic, not a security control (M1 framing)**: the 5-round cap exists to nudge convergence on a direction; it is NOT designed to stop a determined user. A simultaneous reset of both `## Reroll History` (delete the file content) AND `state.design_reroll_round: 0` (manifest edit) will pass every check by design — both counts agree at 0, all defenses see clean state. This is intentional: this is the user's project, the user's taste, the user's time. The harness gates against *accidental* drift — asymmetric edits where one side updates and the other doesn't (the round-2 + round-3 hardening covers this) — not against the user deliberately overriding the budget. The clean override path is `/harness:design explore "<fresh intent>"`: Step 0 resets the counter to 0 and writes a `progress/changelog.md` entry recording the reset (intent + timestamp), giving retrospectives a clear "I deliberately started fresh on this date because X" trail. Manual edits to `.harness/manifest.yaml` are equally the user's prerogative; the tamper-detection clauses below fire only when the manifest count and the `## Reroll History` file count diverge, which is the accident-shaped failure mode worth catching, not the deliberate-reset shape.
