...
vector is a malicious or compromised npm package's documentation surfacing
...
Same defaults as before — **18 enforceable principles** covering code quality, testing (TDD), architecture, and security. Naming conventions and forbidden patterns. Adjust per project but every principle must be testable.

**One principle is mandatory in v3.0+ and must be included verbatim** unless the project's framework genuinely requires silent error catches (rare; document the override in `progress/decisions.md` if so):
...
>
> Exception: tests may use empty catch when the exception is the assertion (e.g., `expect(() => fn()).toThrow()`).
>
> Rationale: silent error swallowing creates ghost-state bugs the Evaluator's black-box testing cannot detect. Bug #6 class (authentication catch swallowing → null user → "feature works unless you've signed out") is the canonical instance.
>
> Once shipped (post-init), only `/harness:constitution-amend` can change this principle.
...
### Build-contract sub-step Y: UI-surface AC for status / state / phase fields (v3.0)
...
- AC-NNN-ui: User can see <field-value-transformation> on `<route>` via
  ...
  `tests/e2e/<NNN-feature-name>/journey.spec.ts > test('<name>')`.
...
### Build-contract sub-step X.1: Populate State-Transition AC table (v3.0)
...
### Build-contract sub-step X.2: Populate Negative-Path Coverage table (v3.0)
...
### Build-contract sub-step Z: User-Journey UI-Surface Brainstorming Pass (v3.0)
...
- Add a UI-surface AC per Build-contract sub-step Y, OR
...
per-journey list, formatted as below (using a standard fenced markdown
block):
...
## Runtime-Verification Coverage (v3.0)
