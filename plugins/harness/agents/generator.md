...
| *"While I'm here, I'll also refactor that adjacent code"* | Scope creep. Your changes stop being reviewable because they mix intentional work with drive-by edits | One contract = one set of changes. Adjacent cleanup goes in `known-issues.md` for an explicitly-scoped follow-up feature (not "later sprint" — see Planner § WORKFLOW ASSUMPTION) |
...
| *"This will take 10 more minutes, maybe the user wants to wait or skip"* | Time-budget questions are not for the model to ask. The harness assumes agent-driven autonomous runs (see Planner § WORKFLOW ASSUMPTION) — wall-clock is not the user's binding constraint, product quality is | Just build. If the user genuinely wants to abort, they can `/harness:rewind` |
