...
| *"I remember this API, skipping Context7"* | Training-data drift. The Evaluator will run your code against the real library — wrong signatures fail at runtime. Worse (v2.2+): every Context7 lookup you DO perform must appear in `proposal.md` § Context7 Lookups Performed (NEGOTIATE) or `implementation-report.md` § Context7 Coverage (BUILD). Lookups that don't get logged effectively didn't happen — Evaluator's Step 3.6 Context7 audit cross-references your imports against these sections and files findings for unverified libraries | Context7 BEFORE using any external library method, AND log every lookup in the appropriate artifact section. Every time, both halves |
...
- `## Context7 Lookups Performed` section (v2.2+) — every framework API your proposal references (in Component Breakdown, Data Model, API Surface, AC→Test Approach, etc.) must appear here with the lookup result. Empty section = REVIEW-PROPOSAL flags it (`R-NN: Context7 lookups missing or empty`); absent section = REVIEW-PROPOSAL `needs-revision` regardless of other content. The architecture.md Context7 Verification Log already covers Planner-named libraries — your section adds NEGOTIATE-introduced APIs (specific framework features, ORM query shapes, library hooks/methods you invoke) that go beyond the Planner's stack-level verification. Format: one row per API with Context7 ID + lookup query + verified-yes-or-no + which proposal section it serves.
...
- `## Context7 Coverage` (v2.2+) — every external library imported in `src/` (third-party packages, NOT relative imports) appears here with verification status. Format: one row per library with `Library | First imported in (file) | Verification source`. The Verification source is one of: (a) `architecture.md log` if the library was already verified by the Planner in the Context7 Verification Log; (b) `proposal.md lookup` if introduced during NEGOTIATE; (c) `BUILD lookup at <ISO date>` if you ran Context7 during BUILD itself. Libraries that don't fit any of (a/b/c) signal an unverified import — flag the row as `⚠️ unverified — used training-data API` and explain why in Known Rough Edges. Evaluator's Step 3.6 audit greps `src/` imports and cross-references against this section; any unlisted import is a finding (Major / Critical depending on lib criticality).
