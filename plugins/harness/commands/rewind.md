...
argument-hint: "<target phase: planning | analyzing | negotiating | building | simulating | evaluating>"
---
...
| `building` | `implementation-report.md`, `simulation-report.md`, `eval-report.md`, `retrospective.md` | spec, final contract, proposal, review |
| `simulating` | `simulation-report.md`, `eval-report.md`, `retrospective.md` | spec, final contract, proposal, review, implementation-report |
...
Orchestrator reads `$ARGUMENTS` and `.harness/manifest.yaml → state.current_feature`. Validate target is one of the six valid phases (`planning | analyzing | negotiating | building | simulating | evaluating`); validate a current feature exists. If either fails, print a concise error and exit.
