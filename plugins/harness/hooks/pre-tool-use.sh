...
# Evaluator's git-archaeology Steps A, B, C, E + the Generator's RED
# FLAGS table cover the same ground. The hook now logs to changelog
# instead of blocking. Restore hard-block in v3.1 if a test-deletion
# incident surfaces during canary or post-release.
if [ -f ".harness/manifest.yaml" ]; then
  ...
  if [ "$PHASE" = "building" ]; then
    ...
    if echo "$TOOL_CMD" | grep -qE 'rm\s+(-[a-zA-Z]+\s+)*[^|;&]*(\.test\.|\.spec\.|__tests__/|\btests?/|\be2e/|playwright\.config)'; then
      ...
      mkdir -p .harness/progress 2>/dev/null
      {
        echo "## ${ts} — pre-tool-use advisory: test-file deletion attempted"
        echo "  Tool: ${TOOL_NAME}"
        echo "  Input: ${TOOL_CMD}"
        echo "  Note: not blocked (advisory mode v3.0+). Evaluator's git-archaeology will re-check."
      } >> .harness/progress/changelog.md 2>/dev/null
