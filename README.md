...
Observe the full pipeline: Planner drafts spec → analyze auto-runs → human approval gate → Generator ↔ Evaluator negotiate the contract → Generator BUILD via TDD with atomic per-FR commits → Generator SIMULATE drives the prod stack and replays cumulative regression → Evaluator EVALUATE reads simulation evidence + 15-min Playwright spot-check, grading against 4 criteria with hard thresholds → tuning check captures any divergence between the Evaluator's judgment and yours → retrospective reconciles spec with what actually shipped → merge.

### For project authors: what you populate (v3.0+)

The harness writes `.env.example` placeholders + journey/fixture tests; **you** populate `.env.local` with real values + customize `init.sh` for your stack. Specifically:
...
- Generator BUILD adds these to `.env.example` automatically when the project has any auth flow:
  ...
  - For multi-tenant: `TEST_FIRM_ID=REPLACE_ME` or `TEST_TENANT_ID=REPLACE_ME` as applicable
- You populate the real values in `.env.local`.
- Customize `init.sh`'s `seed_test_user` function for your auth stack — examples in the template cover Drizzle/Postgres + Clerk admin API; replace the body with your project's seeding logic.
- The journey tests automatically read `process.env.TEST_USER_EMAIL` + `_PASSWORD` to log in. The agent never sees the actual values — same secrets-ownership boundary as production.

**3. Test-fixture lifecycle** (v3.0+, automatic — no action from you)
- Every feature's e2e tests live at `tests/e2e/<NNN-feature-name>/journey.spec.ts` and `fixtures.ts`.
...
|---|---|---|---|---|
| **Planner** | `harness:planner` | Expands a 1-4 sentence prompt into a product-grade spec (PRD + constitution + architecture + evaluator criteria + build contract) | Read, Write, Context7 MCP | PLAN, CLARIFY-QUESTIONS, EDIT (covers AMENDMENT / EDIT / CLARIFY ANSWERS / CONSTITUTION AMENDMENT) |
| **Generator** | `harness:generator` | Negotiates the contract's HOW, then implements it via TDD. Delegates RED→GREEN→REFACTOR to `superpowers:test-driven-development`. Atomic per-FR commits. After BUILD, drives the production stack + cumulative regression in SIMULATE before Evaluator handoff. | Read, Write, Bash, Context7 MCP, Playwright MCP (SIMULATE) | NEGOTIATE, FINALIZE-CONTRACT, BUILD, SIMULATE |
...
*Tool columns reflect each agent's primary tool reach. Per v2.1.1+, no `tools:` allowlist exists in agent frontmatter — subagents inherit the parent Claude Code session's full tool set. The columns above are descriptive (what the agent typically uses), not prescriptive (what they're allowed).*
...
/harness:sprint "<prompt>"
  ...
  ├─ Generator (SIMULATE) — drives prod-mode runtime via Playwright + queries DB at state-transition boundaries + replays every prior shipped feature's journey test → simulation-report.md (authoritative behavioural evidence)
  ├─ Evaluator (EVALUATE) — calibration-mandatory examples.md read → reads simulation-report.md as authoritative behavioural evidence → 15-min Playwright spot-check → Part-A binary gates Part-B numeric → reward-hacking git-archaeology scan → eval-report.md
...
| Command | Purpose |
|---|---|
| `/harness:rewind <phase>` | Archive-based reset to `planning` \| `analyzing` \| `negotiating` \| `building` \| `simulating` \| `evaluating`. Files move to `.archive/TIMESTAMP/`, never deleted. Requires typed confirmation. |
...
| `features/NNN/{amend,clarify,edit}-patches.md` | Planner EDIT mode (with AMENDMENT / CLARIFY ANSWERS / EDIT markers) |
| `features/NNN/pause-questions.md` | Generator BUILD (mid-build clarification) |
| `.harness/constitution-amend-patches.md` | Planner EDIT mode (with CONSTITUTION AMENDMENT marker) |
...
│   ├── constitution.md        # 18 enforceable principles
...
│   ├── simulation-report.md   # Generator SIMULATE's behavioural evidence (VERIFIED|NEEDS-REPAIR)
...
`/harness:rewind <phase>` archives current-phase artifacts to `.harness/features/NNN/.archive/TIMESTAMP/` and resets manifest state. Valid targets: `planning`, `analyzing`, `negotiating`, `building`, `simulating`, `evaluating`. Requires typed confirmation. Not destructive — archived files are recoverable.
